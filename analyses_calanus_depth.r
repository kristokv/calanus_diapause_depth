...
#Test if effect of Chyp found in deeper areas or on average deeper dist of Chyp at same location?
par(mfrow=c(1,2))
for(i in 1:7){
 diff_wmd<-subdat_win[,paste0(lifestages[i+7],"_wmd")] - subdat_win[,paste0(lifestages[i],"_wmd")]
 hist(diff_wmd,main=lifestages[i])
 boxplot(diff_wmd,main="Maxdep")
 print(paste0(lifestages[i+7],"-",lifestages[i],":maxdep"))
 print(summary(diff_wmd))
 print(wilcox.test(diff_wmd,mu=0))
}
...
for(i in 1:length(lifestages)){
    ...
    wmd<-subdat_stage[,paste0(lifestages[i],"_wmd")]
    sd<-subdat_stage[,paste0(lifestages[i],"_sd")]
    #Univariate models:
    for(var in  c("depth","lat")){
      lm_wmd_uni<-lm(wmd~subdat_stage[,var])
      ...
      lm_sd_uni<-lm(sd~subdat_stage[,var])
    ...
    #Multivariate models:
      lm_wmd_multi<-lm(wmd~subdat_stage$depth + subdat_stage$lat)
      ...
      lm_sd_multi<-lm(sd~subdat_stage$depth + subdat_stage$lat)
...
#Plot WMD & SD in separate panels with stages as different colors points (Fig. 7):
